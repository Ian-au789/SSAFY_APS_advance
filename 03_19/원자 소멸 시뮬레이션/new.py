...
def nuclear_reaction(atoms):
    ...
    while atoms:
        ...
        for cur_loc in atoms:
            print(cur_loc)
            next_loc = (cur_loc[0] + direction[atoms[cur_loc][0]][0], cur_loc[1] + direction[atoms[cur_loc][0]][1])

            if next_loc[0] not in range(-1000, 1001) or next_loc[1] not in range(-1000, 1001):  # 범위 밖을 벗어나면 제거
                continue

            if next_loc in atoms:
                if (atoms[cur_loc][0] + atoms[next_loc][0]) % 4 == 1:   # .5초 충돌 발생
                    energy += atoms[cur_loc][1]
                    energy += atoms[next_loc][1]
                    atoms.pop(next_loc)

            else:
                if next_loc in new_atoms:
                    new_atoms[next_loc].append(atoms[cur_loc])          # 충돌 발생
